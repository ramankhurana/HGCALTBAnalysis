void SaveGraphs(  TString energy = "250", TString radiation_length ="_GeV_5_7_X0"){
  ...
  TString dirname = "DirCombine_"+energy+radiation_length;
  gSystem->Exec("mkdir "+dirname);
  
  TString fileout = "OutputCombine_"+energy+radiation_length+".root";
  ...
  ofstream ftext;
  ftext.open("CombineResolution_DifferentAmplitude"+radiation_length+".txt",std::fstream::app);
  for (int iamp=0; iamp<=90; iamp=iamp+5){
    ...
    ftext<<"-1" <<" "<< energy <<" "<<iamp<< " "<<par[1]<<" "<<par[2]<<std::endl;
  }
}
...
void Fitting_CombineRes(){
  ...
  gSystem->Exec("rm CombineResolution_DifferentAmplitude"+radiation_length_+".txt");

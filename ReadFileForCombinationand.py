...
def makeOffsetgraph(cell_,textfilename):
...
ctxtfile= 'CombineResolution_DifferentAmplitude'+radiation_length+'.txt'
...
for iamp in range( len(amplist)):
    mg1 = makeOffsetgraph(str(amplist[iamp]),str(ctxtfile))
    histoname ='EnergyVsWidth_amp_'+str(amplist[iamp])
